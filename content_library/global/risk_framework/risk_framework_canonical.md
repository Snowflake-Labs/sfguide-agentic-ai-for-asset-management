---
doc_type: risk_framework
linkage_level: global
variant_id: risk_management_framework
word_count_target: 2850
placeholders:
  required: []
---

# Snowcrest Asset Management — Risk Management Framework

### Approved By

Board of Directors
### Effective Date

1 January 2024
### Review Date

Annual
### Owner

Chief Risk Officer
### Classification

Internal — Confidential

---

## EXECUTIVE SUMMARY

This Risk Management Framework establishes Snowcrest Asset Management's comprehensive approach to identifying, assessing, managing, and monitoring risks across the organization. The Framework defines risk governance, establishes risk appetite, provides systematic risk methodologies, and ensures appropriate oversight and reporting.

Our risk management philosophy balances pursuit of attractive investment returns with protection of client capital and firm reputation. We view effective risk management as competitive advantage enabling us to take calculated risks where compensated whilst avoiding unnecessary or uncompensated risks.

---

## 1. INTRODUCTION AND PURPOSE

### Framework Objectives

This Risk Management Framework serves to:
- Define clear risk governance structure with roles and responsibilities
- Establish risk appetite statement aligned with business strategy
- Provide systematic risk identification and assessment methodologies
- Set standards for risk mitigation, controls, and monitoring
- Ensure appropriate risk reporting to stakeholders and Board
- Create risk-aware culture throughout the organization

### Scope and Application

The Framework applies to all business activities, operations, and organizational levels including investment management, client service, operations, technology, and support functions. All employees share responsibility for risk awareness and management within their respective areas.

---

## 2. RISK GOVERNANCE

### Three Lines of Defense Model

### First Line — Business Operations

- Portfolio managers and investment teams
- Client service and operations departments
- **Responsibility**: Own and manage risks in daily operations, implement controls, escalate issues

### Second Line — Risk Oversight

- Risk Management function led by Chief Risk Officer
- Compliance function led by Chief Compliance Officer
- **Responsibility**: Establish policies, provide oversight, monitor adherence, independent challenge

### Third Line — Independent Assurance

- Internal Audit (outsourced)
- External Auditors
- **Responsibility**: Independent assessment, validation, assurance

### Governance Structure

### Board of Directors

- Ultimate risk oversight responsibility
- Approves Risk Management Framework and Risk Appetite Statement
- Reviews quarterly risk reports and material risk events
- Challenges management on risk matters

**Risk Committee** (Board Committee):
- Oversees framework implementation
- Reviews significant risk exposures and risk appetite compliance
- Challenges risk management practices and effectiveness
- Meets quarterly with management and annually with auditors

### Chief Risk Officer

- Day-to-day risk management leadership
- Reports to CEO with direct access to Board Risk Committee
- Independent of revenue-generating functions
- Escalation authority for material risks

### Investment Committee

- Reviews portfolio risk profiles before approval
- Approves concentration limit exceptions
- Monitors risk-adjusted performance
- Validates investment process compliance

### Compliance Committee

- Monitors regulatory and compliance risks
- Reviews testing results and violations
- Approves policy changes and enhancements

---

## 3. RISK APPETITE STATEMENT

Snowcrest Asset Management's risk appetite defines the amount and types of risk we are willing to accept in pursuit of strategic objectives.

### Qualitative Risk Appetite

We will:
- Maintain reputation for integrity, fiduciary excellence, and ethical conduct
- Protect client assets through prudent risk management and strong controls
- Comply fully with all regulatory requirements and industry standards
- Accept calculated investment risks within defined parameters and client mandates
- Minimize operational, compliance, and reputational risks
- Maintain zero tolerance for fraud, market abuse, or regulatory violations

### Quantitative Risk Metrics

| Risk Type | Metric | Limit | Rationale |
|-----------|--------|-------|-----------|
| **Market Risk** | Portfolio VaR (95%, 1-day) | 2% of portfolio value | Capital protection |
| **Concentration** | Single issuer | 7% maximum | Diversification |
| **Credit Risk** | Max single issuer (FI) | 5% of FI allocation | Default risk management |
| **Liquidity Risk** | Min liquid assets | 90% tradeable 7 days | Redemption capacity |
| **Operational** | Max operational losses | 0.1% AUM annually | Process quality |
| **Compliance** | Regulatory breaches | Zero tolerance | License protection |

### Risk Capacity

Risk capacity reflects our financial ability to absorb potential losses:
- Strong capital position with ${{FIRM_CAPITAL}}M net worth
- Operating cash flow adequate for business requirements
- Professional indemnity insurance coverage of ${{INSURANCE_COVERAGE}}M
- Credit facilities available for extraordinary circumstances

---

## 4. RISK TAXONOMY

### Market and Investment Risks

### Equity Risk

Price volatility of equity securities due to market movements, company-specific events, or sector dynamics.

### Interest Rate Risk

Bond price sensitivity to interest rate changes affecting fixed income portfolio values.

### Credit Risk

Issuer default risk, credit spread widening, rating downgrades impacting bond values.

### Liquidity Risk

Inability to trade positions at fair prices or within required timeframes.

### Currency Risk

Foreign exchange fluctuations affecting international investments (generally hedged to base currency).

### Concentration Risk

Excessive exposure to single securities, issuers, sectors, or geographies.

### Operational Risks

### Process Risk

Failures in trade execution, settlement, reconciliation, or other operational processes causing errors or losses.

### Technology Risk

System failures, outages, cybersecurity breaches, or data integrity issues disrupting operations.

### Human Error

Mistakes in data entry, calculation errors, miscommunications, or judgment failures.

### Third-Party Risk

Service provider failures including custodians, administrators, or technology vendors.

### Fraud Risk

Internal fraud, external fraud, or cyber-fraud affecting firm or client assets.

### Compliance and Regulatory Risks

### Regulatory Risk

Non-compliance with securities regulations, resulting in regulatory sanctions, fines, or license restrictions.

### Legal Risk

Contractual disputes, litigation, or legal claims against the firm or personnel.

### Fiduciary Risk

Breach of fiduciary duties including conflicts of interest or unsuitable investment recommendations.

### Business and Strategic Risks

### Strategic Risk

Business strategy execution failures, market positioning errors, or competitive disadvantage.

### Reputational Risk

Damage to firm reputation from investment losses, compliance failures, or public controversies.

### Business Continuity Risk

Inability to maintain operations during disruptions (natural disasters, pandemics, cyber-attacks).

---

## 5. RISK IDENTIFICATION AND ASSESSMENT

### Risk Identification Methods

### Risk and Control Self-Assessments (RCSAs)

Quarterly self-assessments by department heads identifying risks, evaluating controls, and documenting gaps.

### Investment Risk Analysis

Pre-investment due diligence, ongoing position monitoring, stress testing, and scenario analysis identifying investment risks.

### Compliance Monitoring

Automated surveillance, exception reporting, and testing programmes identifying compliance and regulatory risks.

### Incident Tracking

Operational errors, near-misses, and risk events analyzed for root causes and systemic issues.

### External Intelligence

Regulatory guidance, industry developments, cyber threat intelligence, and market risk indicators.

### Risk Assessment and Prioritization

### Risk Impact Assessment

Potential severity measured in financial loss, regulatory consequence, reputational damage, or operational disruption.

### Probability Assessment

Likelihood evaluated based on historical frequency, control effectiveness, and risk environment.

### Risk Rating Matrix

Combining impact and probability yields risk rating (High/Medium/Low) informing prioritization and mitigation strategies.

---

## 6. RISK MITIGATION AND CONTROL

### Preventive Controls

- Authorization and approval requirements for key decisions
- Segregation of duties in trading, operations, and compliance
- System access controls and authentication
- Investment guidelines and automated pre-trade checks
- Training and competency requirements

### Detective Controls

- Post-trade monitoring and exception reporting
- Daily position and exposure reporting
- Reconciliations (cash, positions, performance)
- Compliance testing and surveillance
- Performance attribution review

### Corrective Controls

- Error correction and client remediation procedures
- Breach escalation and remediation protocols
- Root cause analysis and process improvements
- Disciplinary procedures for violations
- Continuous monitoring of remediation effectiveness

### Risk Transfer

### Insurance Coverage

- Professional indemnity insurance (errors & omissions)
- Directors and officers liability insurance
- Cyber liability and data breach insurance
- Fidelity bond covering employee dishonesty

---

## 7. RISK MONITORING AND REPORTING

### Daily Monitoring

- Real-time portfolio VaR and concentration monitoring
- Compliance alerts for guideline breaches
- Trading error detection and reporting
- Cash and liquidity adequacy

### Weekly Reporting

- Risk dashboard to CRO covering key metrics
- Breach and exception summary
- New client onboarding and mandate reviews

### Monthly Reporting

- Risk Committee report with KRI trends
- Operational risk incidents and resolutions
- Compliance testing results
- Performance attribution and risk factor exposures

### Quarterly Reporting

- Board Risk Committee comprehensive report
- Risk appetite compliance assessment
- Stress testing and scenario analysis results
- Emerging risks and forward-looking assessment

### Annual Reporting

- Comprehensive risk review covering all categories
- Framework effectiveness assessment
- Recommendations for enhancements
- Regulatory examination preparation

---

## 8. KEY RISK INDICATORS (KRIs)

### Investment Risk KRIs

- Portfolio VaR trends and limit utilization
- Concentration levels and threshold breaches
- Tracking error and active risk metrics
- Largest drawdowns and recovery periods

### Operational Risk KRIs

- Trade error rates and operational losses
- System availability and incident counts
- Reconciliation breaks and aging
- Client complaint frequency

### Compliance Risk KRIs

- Regulatory examination findings
- Policy violation frequency and severity
- Compliance testing failure rates
- Personal trading violations

---

## 9. STRESS TESTING AND SCENARIO ANALYSIS

### Quarterly Stress Tests

- Market crash scenario (-30% equity, +100bps rates)
- Credit crisis (spread widening, ratings downgrades)
- Liquidity crisis (reduced trading volumes, wider bid-asks)
- Customized scenarios based on current risk environment

### Annual Comprehensive Scenarios

- Historical crisis replays (2008 financial crisis, COVID-19, etc.)
- Forward-looking scenarios (geopolitical events, policy shocks)
- Reverse stress testing (what scenarios break the business?)

---

## 10. FRAMEWORK REVIEW AND CONTINUOUS IMPROVEMENT

### Annual Framework Review

- Board approval required for material changes
- Risk appetite reassessment
- Effectiveness evaluation
- Industry benchmarking

### Continuous Improvement

- Lessons learned from risk incidents and near-misses
- Regulatory guidance and examination findings
- Industry best practice adoption
- Technology and analytical capability enhancements

---

### Framework Approval

Board of Directors: 15 December 2023
Effective Date: 1 January 2024  
Next Review: December 2024

*Snowcrest Asset Management Risk Management Framework — Confidential*

