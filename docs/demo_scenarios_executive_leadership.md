...
> **Skills**: `pdf-report-generation`, `portfolio-name-resolution`, `executive-briefing`, `competitor-ma-analysis`, `data-lineage-explanation`, `audience-adaptive-narrative`  
...
- **PDF Generation**: Branded internal document with SAM logo
...
- `search_internal_docs` (Cortex Search) - Investment philosophy and strategic positioning
...
## Executive Copilot - Performance Attribution Briefing

### Business Context Setup

**Persona**: Sarah Chen, Head of Asset Management at Simulated Asset Management  
**Business Challenge**: The board asks "why did we underperform this quarter?" Sarah needs a decomposition of active return into allocation vs selection effects — not just the return number. Traditional performance teams take days to produce this analysis.  
**Value Proposition**: Instant attribution decomposition accessible from the executive's primary tool, without needing to switch to a specialist attribution system.

**Agent**: `executive_copilot`  
**Data Available**: Multi-level Brinson attribution (sector, country, industry), linked QTD/YTD periods

### Demo Flow

**Scene Setting**: Sarah is 10 minutes before the board meeting. A director has emailed asking "Why did Flagship underperform by 60bps this quarter?" She needs a clear answer fast.

##### Step 1: Board-Ready Attribution Briefing

**Skill Activated**: `attribution-report-generator` (executive tier — detects "board meeting" context)
**Workflow**: Sector attribution → anomaly check → executive-format synthesis
**Demo Value**: One prompt generates a complete board-ready briefing with the skill orchestrating multiple tools automatically

**User Input**: 
```
I need a board-ready attribution briefing for Flagship this quarter. Why did we underperform?
```

**Tools Used**:
- `server_skill` — Loads attribution-report-generator skill (detects executive audience)
- `brinson_analyzer` (Cortex Analyst) - Sector attribution + linked QTD
- `anomaly_detector` (Cortex Analyst) - Checks for critical risk flags

**Expected Response**:
- The skill runs its 5-step workflow automatically, presenting executive-format output:
  - 1-sentence headline with the active return
  - Top 3 drivers as bullet points
  - Any HIGH severity risk flags
- STOPPING POINT: "Would you like country-level detail, or is this sufficient for the board?"

**Talking Points**:
- One question triggered a multi-tool workflow (attribution + anomaly check + synthesis)
- The skill detected "board-ready" and automatically applied executive formatting
- No tables, no basis points — just themes and drivers, exactly what a board needs
- Provides a board-ready answer in seconds, not days

**Key Features Highlighted**: 
- attribution-report-generator skill with automatic audience detection
- Cross-pollinated attribution in the executive's own agent
- Stopping point offers country drill-down without requiring a separate question

##### Step 2: Country Drill-Down (Stopping Point Continuation)

**Presenter Transition**:
> "The skill offered a country drill-down at the stopping point. The director wants to know if this is a US problem. Instead of a separate analysis request, we just continue the workflow..."

**User Input**: 
```
Which countries contributed most to the underperformance?
```

**Tools Used**:
- `brinson_analyzer` (Cortex Analyst) - Queries multi_level_detail with grouping_dimension = 'COUNTRY'

**Expected Response**:
- Country-level attribution showing US vs international contributions
- Identification of whether underperformance is broad-based or country-specific
- Note: "For deeper factor analysis or stress testing, the Portfolio Manager Co-Pilot agent provides specialist capabilities"

**Talking Points**:
- Multi-level attribution gives the executive the ability to answer follow-up questions in real time
- The agent naturally suggests escalation to the specialist for deeper analysis
- This is the "glass ceiling" pattern: executive gets summary + first drill-down, specialist gets the full toolkit

#### Scenario Wrap-up

**Business Impact Summary**:
- **Board Readiness**: Attribution answer in seconds, not days
- **Self-Service**: Executive doesn't need to request a report from the performance team
- **Escalation Path**: Clear handoff to Portfolio Manager Co-Pilot for deeper analysis

**Technical Differentiators**:
- **Cross-Pollinated Data**: Same attribution views serve multiple agents at different depths
- **Multi-Level**: Sector + country decomposition from one expanded semantic view
- **Linked Periods**: QTD/YTD figures account for compounding via Frongello method
...
| `financial_analyzer` | Cortex Analyst | Competitor intelligence from SEC filings |
| `implementation_analyzer` | Cortex Analyst | Client mandate requirements and constraints |
| `search_internal_docs` | Cortex Search | Investment philosophy and strategy documents |
| `search_external_docs` | Cortex Search | Competitor news and M&A announcements |
...
- `SAM_RESEARCH_VIEW`: Competitor data from SEC filings (existing)
- `SAM_PORTFOLIO_VIEW`: Portfolio analytics for holdings detail (existing)
