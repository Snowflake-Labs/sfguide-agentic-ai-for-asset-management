# SAM Demo - Sales Advisor Scenarios

Complete demo scenarios for the Sales Advisor with step-by-step conversations, expected responses, and data flows.

> **Scenario**: `sales_advisor`  
> **Agent**: `AM_sales_advisor`  
> **Demo Surfaces**: Snowflake Intelligence  
> **Skills**: `pdf-report-generation`, `portfolio-name-resolution`, `quarterly-client-letter`, `client-review-preparation`, `regulatory-lookup`, `audience-adaptive-narrative`  
...
**Persona**: James Mitchell, Client Relationship Manager at Simulated Asset Management  
...
**Tools Used**:
- `search_internal_docs` (Cortex Search) - Search ESG investment philosophy
...
**Tools Used**:
- `search_internal_docs` (Cortex Search) - Search ESG policy requirements
...
**Key Features Highlighted**: 
- **SAM_PORTFOLIO_VIEW**: Complete portfolio data including ESG dimensions
...
- **Brand Excellence**: SAM branding reinforces institutional quality
...
### Sales Advisor - Performance Story for Prospect Meeting

#### Business Context Setup

**Persona**: James Mitchell, Client Relationship Manager at Simulated Asset Management  
**Business Challenge**: A prospect wants to understand SAM's recent investment performance before making an allocation decision. James needs a compelling performance narrative with YTD attribution data, framed in prospect-friendly language — not the technical Brinson terminology used internally.  
**Value Proposition**: AI-powered performance storytelling that combines linked attribution data with client-friendly framing, enabling compelling prospect conversations backed by real performance decomposition.

**Agent**: `sales_advisor`  
**Data Available**: Multi-level Brinson attribution with linked QTD/YTD periods

#### Demo Flow

**Scene Setting**: James has a prospect meeting in 30 minutes. The prospect is considering SAM ESG Leaders and wants to understand the YTD performance story.

##### Step 1: YTD Performance Story

**Skill Activated**: `multi-level-attribution` (detects "prospect" → client audience tier, uses linked YTD)
**Workflow**: Identify portfolio + period → linked YTD attribution → client-friendly presentation
**Demo Value**: The skill automatically detects the prospect context and frames attribution positively without Brinson jargon

**User Input**: 
```
I'm meeting a prospect about SAM ESG Leaders Global Equity. Give me the YTD performance story — what's driven the returns?
```

**Tools Used**:
- `server_skill` — Loads multi-level-attribution skill (detects "prospect" → client tier)
- `brinson_analyzer` (Cortex Analyst) - Queries linked YTD attribution by sector

**Expected Response**:
- YTD return vs benchmark framed positively
- Top 3 sector contributors in plain language: "Strong stock picking in Technology" not "Selection effect +85bps"
- Strategy context: how the attribution aligns with the ESG investment thesis
- No Brinson terminology — the skill's client tier formatting removes all jargon

**Talking Points**:
- The skill detected "prospect" and automatically applied client-friendly formatting
- Same underlying data that the PM sees in basis points — but framed for a sales conversation
- Linked YTD figures account for compounding — these are fact-sheet-ready numbers
- The agent connects attribution to the strategy narrative without being asked

##### Step 2: Side-by-Side Portfolio Comparison

**Presenter Transition**:
> "The prospect is also considering our Flagship fund. Let me pull a quick comparison so James can discuss both options..."

*Reasoning: Prospects often evaluate multiple products. Side-by-side attribution comparison helps position each strategy's strengths.*

**User Input**: 
```
How does the ESG Leaders performance compare to our Flagship portfolio? The prospect is considering both.
```

**Tools Used**:
- `brinson_analyzer` (Cortex Analyst) - Queries linked YTD attribution for both portfolios

**Expected Response**:
- Side-by-side YTD attribution for both portfolios
- Key differentiators: "ESG Leaders generates alpha primarily through selection (stock-picking) while Flagship relies more on allocation (sector bets)"
- Recommendation framing: "For a client focused on ESG integration, the selection-driven alpha in ESG Leaders aligns well with their mandate"

**Talking Points**:
- Cross-portfolio comparison from a single query
- The sales agent frames the comparison competitively — not "one is better" but "each excels differently"
- Attribution data supports the sales narrative with hard numbers

#### Scenario Wrap-up

**Business Impact Summary**:
- **Prospect Readiness**: Performance story prepared in minutes with attribution backing
- **Competitive Positioning**: Side-by-side comparison highlights each strategy's strengths
- **Consistent Messaging**: AI ensures attribution is framed consistently across all client communications

**Technical Differentiators**:
- **Cross-Pollinated Attribution**: Sales agent accesses the same institutional-grade data as the PM and CIO
- **Audience-Aware Framing**: No Brinson jargon — prospect-friendly narrative automatically
- **Linked Periods**: YTD figures suitable for fact sheets and marketing materials
...
**Scene Setting**: James is preparing a comprehensive annual review briefing for Corporate Pension Trust 017, SAM's largest institutional client invested in the Technology & Infrastructure portfolio. This requires performance analysis, professional formatting, investment philosophy integration, and full compliance review—a complete demonstration of all Sales Advisor capabilities.

##### Step 1: Performance Foundation
**User Input**: 
```
I need to prepare a comprehensive annual review briefing for Corporate Pension Trust 017's SAM Technology & Infrastructure portfolio. Start with full performance analysis for the last calendar year, including returns, quarterly breakdown, top holdings as of year-end, and sector allocation.
```
...
**Expected Response**:
- **Last Year Performance**: Full calendar year returns vs benchmark
- **Quarterly Breakdown**: Q1-Q4 performance by quarter
- **Top Holdings (Year-End)**: Largest positions with weights as of December 31st
...
**Key Features Highlighted**: 
- **SAM_PORTFOLIO_VIEW**: Complete portfolio analytics for institutional reporting
- **Automated Analysis**: Complex multi-period analysis in seconds

##### Step 2: Professional Template Structure

**Presenter Transition**:
> "Now I have last year's comprehensive performance data. For our largest client's annual review, this needs to be in our most professional format—the annual client presentation template with full executive structure..."
...
**Tools Used**:
- `search_internal_docs` (Cortex Search) - Executive briefing template
...
**Tools Used**:
- `search_internal_docs` (Cortex Search) - Technology investment philosophy
...
**User Input**: 
```
Add complete regulatory compliance including all required disclosures, performance disclaimers, risk warnings, and fiduciary language appropriate for our largest institutional relationship. Include relevant MiFID II and FCA regulatory disclosure requirements.
```

**Tools Used**:
- `search_internal_docs` (Cortex Search) - Compliance and regulatory requirements from internal policies
- `search_regulations` (Cortex Search) - MiFID II investor protection and FCA Consumer Duty disclosure requirements
...
- **Regulatory Compliance**: MiFID II required disclosures for professional client communications
- **FCA Consumer Duty**: Client outcome obligations and fair value statements
- **Conflict Disclosures**: Any relevant conflict of interest statements

**Talking Points**:
- **Institutional Standards**: Compliance language for sophisticated clients backed by regulation text
- **Regulatory Excellence**: Disclosures grounded in actual MiFID II and FCA regulation
- **Fiduciary Care**: Demonstrates SAM's commitment to client protection with regulatory citations

**Key Features Highlighted**: 
- **SAM_POLICY_DOCS**: Complete compliance and regulatory library
- **SAM_REGULATORY_DOCS**: MiFID II and FCA Consumer Duty regulation text for disclosure requirements
- **Institutional Compliance**: AI applies highest compliance standards with regulatory backing
...
- **PDF Generation**: Branded PDF with SAM logo and client-appropriate formatting
...
**Tools Used**:
- `quantitative_analyzer` (Cortex Analyst) - Portfolio performance from SAM_PORTFOLIO_VIEW
...
- `search_internal_docs` (Cortex Search) - Client meeting template format
...
- **Cross-View Orchestration**: Seamless combination of client data (SAM_EXECUTIVE_VIEW) with portfolio analytics (SAM_PORTFOLIO_VIEW)
...
**Tools Used**:
- `search_internal_docs` (Cortex Search) - Get client onboarding welcome template
...
**Tools Used**:
- `search_internal_docs` (Cortex Search) - Get ESG investment philosophy
...
**Tools Used**:
- `search_internal_docs` (Cortex Search) - Get client retention playbook
...
**Tools Used**:
- Synthesis of previous queries
- `search_internal_docs` (Cortex Search) - Get value proposition content
...
**Note on Value Proposition Content**: The `search_internal_docs` tool (filter: philosophy_docs) will return documents including "Simulated Asset Management — Brand Messaging Guidelines" which contains comprehensive value proposition content under the "Core Value Propositions" section, covering Innovation Leadership, ESG Excellence, and Performance Focus messaging.
...
**Tools Used**:
- `search_internal_docs` (Cortex Search) - Get product catalog
...
- `search_internal_docs` (Cortex Search) - Get product philosophy alignment
...
- `search_internal_docs` (Cortex Search) - Report template structure, investment philosophy, policy information
- `search_regulations` (Cortex Search) - Regulatory disclosure requirements for client communications
- `institutional_holdings_analyzer` (Cortex Analyst) - Institutional ownership context
...
- **Cross-View Orchestration**: SAM_EXECUTIVE_VIEW + SAM_PORTFOLIO_VIEW in single query
...
### Sales Advisor - Institutional Ownership Intelligence

#### Demo Scenario

**User Input**: 
```
For our prospect meeting with a large pension fund, who are the top institutional holders of Microsoft and how has ownership changed?
```

**Tools Used**:
- `institutional_holdings_analyzer` (Cortex Analyst) - 13F holdings for Microsoft
- `quantitative_analyzer` (Cortex Analyst) - Our portfolio positioning in Microsoft

**Expected Response**:
- Top institutional holders (Vanguard, BlackRock, etc.) with positions
- Quarterly ownership changes
- Comparison with our portfolio's position
- Talking points for prospect meeting

**Key Features Highlighted**: 
- SEC 13F institutional ownership data for client conversations
- Competitive intelligence on who else holds the stock
- Integration with portfolio analytics for differentiated positioning
